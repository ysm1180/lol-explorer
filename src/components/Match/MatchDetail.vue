    ...
    <v-layout column>
      <v-layout column lighten-5 :class="gameInfo.teams['100'].isWin?'blue':'red'">
        <v-flex text-xs-left pa-2>
          ...
          >블루팀({{gameInfo.teams['100'].isWin?'승리':'패배'}})</span>
          <span
            class="font-weight-bold mr-4"
          >{{gameInfo.teams['100'].totalKills}} / {{gameInfo.teams['100'].totalDeaths}} / {{gameInfo.teams['100'].totalAssists}}</span>
          ...
          <v-layout fill-height justify-space-between>
            <v-flex class="participant-row-area1" fill-height>
              <v-layout fill-height align-center>
                ...
                >
                  ...
                  >{{participant.stats.champLevel}}</span>
                </v-img>
              </v-layout>
            </v-flex>
            <v-flex class="participant-row-area2" fill-height>
              <v-layout fill-height align-center>
                ...
                >{{participant.player.summonerName}}</span>
              </v-layout>
            </v-flex>
            ...
            <v-flex class="participant-row-area4" fill-height>
              <v-layout column fill-height justify-center>
                ...
                >평점 {{((participant.stats.kills + participant.stats.assists) / participant.stats.deaths).toFixed(2)}}</span>
                <span
                  class="font-size-small"
                >{{participant.stats.kills}}/{{participant.stats.deaths}}/{{participant.stats.assists}} ({{((participant.stats.kills + participant.stats.assists) * 100 / gameInfo.teams['100'].totalKills).toFixed(2)}}%)</span>
              </v-layout>
            </v-flex>
            <v-flex class="participant-row-area5" fill-height>
              <v-layout column fill-height justify-center>
                <span
                  class="font-size-small font-weight-bold"
                >골드 {{participant.stats.goldEarned | gold}}</span>
                <span
                  class="font-size-small"
                >CS {{participant.stats.totalMinionsKilled + participant.stats.neutralMinionsKilled}}</span>
              </v-layout>
            </v-flex>
      ...
      <v-layout column red lighten-5 :class="gameInfo.teams['200'].isWin?'blue':'red'">
        <v-flex text-xs-left pa-2>
          ...
          >레드팀({{gameInfo.teams['200'].isWin?'승리':'패배'}})</span>
          <span
            class="font-weight-bold mr-4"
          >{{gameInfo.teams['200'].totalKills}} / {{gameInfo.teams['200'].totalDeaths}} / {{gameInfo.teams['200'].totalAssists}}</span>
          ...
          <v-layout fill-height justify-space-between>
            ...
            <v-flex class="participant-row-area4" fill-height>
              <v-layout column fill-height justify-center>
                ...
                <span
                  class="font-size-small"
                >{{participant.stats.kills}}/{{participant.stats.deaths}}/{{participant.stats.assists}} ({{((participant.stats.kills + participant.stats.assists) * 100 / gameInfo.teams['200'].totalKills).toFixed(2)}}%)</span>
              </v-layout>
            </v-flex>
            <v-flex class="participant-row-area5" fill-height>
              <v-layout column fill-height justify-center>
